...
from .models import patient , doctor , diseaseinfo , consultation , Hospital
...
def home(request):
   ...
   if request.method == 'GET':
      # news api
      news_api_request = requests.get('http://newsapi.org/v2/top-headlines?country=us&category=health&apiKey=f7fe0f51f238423d92e7ef9f820adaeb')
      api = json.loads(news_api_request.content)
      news = 'saroj'
      
      if request.user.is_authenticated:
        
         return render(request,'homepage/index.html', {'news': news, 'api': api})
...
def covid(request):
    ...
    while(data):
        try:
            ...
            data = True
    return render(request , 'remedieshtml/covid.html' ,
     ...
     return render(request,'patient/checkdisease/checkdisease.html', {"list2":alphabaticsymptomslist})
      ...
      if (inputno == 0 ) :
       ...
       #consult_doctor codes----------
        ...
        return JsonResponse({'predicteddisease': predicted_disease ,'confidencescore':confidencescore , "consultdoctor": consultdoctor, "hospital":hospital})
...
def hospital(request, id):
    post = Hospital.objects.filter(hospital_id = id)[0]
    print(post)
